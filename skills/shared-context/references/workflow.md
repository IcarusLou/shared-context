...
- **Confirm it yourself**: `candidate_confirm`, `decision_source: "agent_policy"`, no `edits`, and only a `ready_for_review` row whose `top_assessment.relation` is `novel` or `supports`. The server checks the same permission surface and refuses anything outside it as `auto_confirm_not_permitted` — a missing permission, not a malformed request: do not change fields and retry; move that Candidate to the third tier.
