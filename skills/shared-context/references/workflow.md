...
- **Confirm it yourself**: `candidate_confirm`, `decision_source: "agent_policy"`, no `edits`, and only a row whose `auto_confirmable` is `true` — a pending Review at `candidate_status: "ready_for_review"` whose `top_assessment.relation` is `novel` or `supports`. `ready_for_review` says the row wants a reviewer; `auto_confirmable` says you may be that reviewer. They differ most often at `candidate_status: "needs_space_review"`, where no existing Primary Space was elected and the Candidate must be placed first. The server enforces the predicates it derives `auto_confirmable` from and refuses anything else as `auto_confirm_not_permitted`, whose message names the missing step — a missing permission, not a malformed request: do not change fields and retry; move that Candidate to the third tier.
