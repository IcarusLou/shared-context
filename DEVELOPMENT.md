...
- Enabled PostTool Hook 不运行 Git、Scanner、Graph rebuild，也不发起 ArtifactFocusQuery；它先验证全部结构化路径都属于 lease 允许的 Repository。归属通过的 File observation 才形成 Breadcrumb，可识别 Test/Check/Lint 执行才形成非事实、非定位的 TestOutcome TaskSignal；sibling、mixed、ambiguous 或 unsafe 事件整条丢弃。TaskSignal 可影响 Working Intent retrieval，但不是工程 Evidence；setup、doctor 与显式 Runtime open 才验证 Catalog。
