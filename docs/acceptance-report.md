...
| M3 — Engineering Graph | IMPLEMENTED | Fixed cross-crate/E2E oracle proves multi-language Artifact discovery, relocation/rename recovery, exact graph retrieval, 1–2 hop cross-platform Context, diagnostics, fallback, rebuild equivalence, and snapshot/budget bounds |
...
| File move and Symbol rename recover without mutating References | PROVEN | M3 oracle moves a JS file and renames a TS Symbol, then proves content/semantic resolution lands on the fixed new paths while original Reference locators remain unchanged |
