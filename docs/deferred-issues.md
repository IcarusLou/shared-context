...
57. **`engineering_workflows::public_mcp_artifact_focus_is_query_scoped_across_six_kinds_and_hot_path` 在并发套件下偶发**:`assert_eq!(rebuilt.status_counts.resolved, cases.len())` 实测出现 8/9(少一条解析),单独重跑与在基线 commit 上整套重跑都通过,所以是负载相关的偶发而非本轮引入。与 #47(hook_fail_open 并发 flake)同类但不同根。**升级条件**:复现率上升,或能稳定在单套件内复现——届时查 `association_rebuild` 的扫描是否对 mtime/时序有隐含依赖。
