...
3. **推理**：MCP 进程内 ort（ONNX Runtime）推理，模型加载惰性（首次带 embedding 的查询触发）；索引侧在投影更新时为 accepted revision 计算并缓存向量（`SEARCH_RANKING_VERSION` bump 触发重建）；向量存 index.sqlite 新表，按 revision_id 键控。
