...
- `[retrieval]`：可选的 embedding 召回通道（ADR-0004）。**两个键都不写就是默认：通道完全不存在，检索与加入该通道之前逐字节一致，零磁盘、零内存、零延迟开销。** 两个键必须同时写、且都必须是绝对路径；只写一个视为配置错误（`sctx doctor` 会 Warning，通道保持关闭）。**不需要手工写这一节**：`sctx embedding install`（见 3.7）会下载、校验、自检后替你写好；下面的说明是给自备模型或非 macOS 平台的用户看的。
  ...
  - 查询侧的那路余弦（`QWEN3_SEMANTIC_SIMILARITY_FLOOR_BASIS_POINTS` / bge-m3 的 0.52 一族，标定过程见 ADR-0004）**已从自动注入退役**，只剩显式 Pack 读取还在用。退役的理由是实测而不是偏好：在真实装机上，五条真实 Working Intent 的查询侧 top-1 每一次都来自不相干主题，同时整体排序几乎完美——它排得好、准入得差，而自动注入要的恰恰是准入判定（ADR-0007）。查询向量的进程内 LRU（64 条）与超时后仍回写缓存的行为保持不变。
